package com.avinfo.avmusic.services;
...
import com.avinfo.avmusic.Handlers.NotificationHandler;
...
public class ServicePlayMusic extends MediaBrowserServiceCompat
    ...
    public static final String ACTION_CMD = "com.avinfo.avmusic.ACTION_CMD";

    public static final String CMD_NAME = "AVPlayer";
    ...
    private static final int STOP_DELAY = AppMain.settings.get("sleepTimer", 5) * 60000;
    ...
    private BroadcastReceiver mNoisyReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (AudioManager.ACTION_AUDIO_BECOMING_NOISY.equals(intent.getAction())) {
                if (AppMain.settings.get("pauseHeadphoneUnplugged", true)) {
                    pausePlayer();
                }
                ...
                switch (state) {
                    case 0:
                        ...
                        break;
                    case 1:
                        if (AppMain.settings.get("resumeHeadphonePlugged", true)) {
                            unpausePlayer();
                        } else pausePlayer();
                        break;
                }
                ...
                if (event.getRepeatCount() == 0 && action == KeyEvent.ACTION_DOWN) {
                    ...
                    switch (keycode) {
                        case KeyEvent.KEYCODE_HEADSETHOOK:
                            if (!AppMain.settings.get("headphoneControl", true))
                                break;
    ...
    public void stopMusicPlayer() {
        if (player == null)
            return;
        AppMain.mainMenuHasNowPlayingItem = false;
        player.stop();
        player.release();
        player = null;
    ...
    public void setList(ArrayList<Song> theSongs) {
        if (AppMain.settings.get("savePlaylist", true))
            UtilsRecent.saveLastPlaylist(getApplicationContext(), theSongs);
        songs = theSongs;
        currentSongPosition = 0;
    }
    ...
    private void setMediaSessionMetaData() {
        mMediaSessionCompat.setMetadata(new MediaMetadataCompat.Builder()
                .putBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART, UtilsExtra.getBitmapfromAlbumId(getApplicationContext(), currentSong))
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        if (currentSongPosition == 0) {
            if (AppMain.settings.get("repeat_list", true))
                playSong();
            else
                destroySelf();
            return;
    ...
    public void playSong() {
        ...
        if (AppMain.settings.get("saveRecent", true))
            UtilsRecent.addsong_toRecent(getApplicationContext(), songToPlay);
        if (AppMain.settings.get("saveCount", true))
            UtilsRecent.addcountSongsPlayed(getApplicationContext(), songToPlay);
    ...
    private static class DelayedStopHandler extends Handler {//延迟停止服务handler
        ...
        public void handleMessage(Message msg) {
            ...
            if (service != null && AppMain.mainMenuHasNowPlayingItem) {
